...
export default function AppSubNavbar({ initialIsExpanded, onCollapse, user }: AppSubNavbarProps) {
  ...
  const linkContainerClasses = isExpanded 
    ? (isDesktop ? 'flex flex-wrap items-center gap-2 p-2 justify-center' : 'grid grid-cols-2 gap-2 p-2') // Grid for mobile expanded, centered for desktop expanded
    : (isDesktop 
      ? 'flex flex-wrap items-center gap-2 p-2 justify-center' // Centered for desktop non-expanded
      : 'hidden');
