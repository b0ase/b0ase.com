...
import { FaPlusSquare, FaUsers, FaProjectDiagram, FaUserPlus, FaBriefcase, FaChevronDown, FaChevronUp, FaRocket, FaCubes, FaUserSecret, FaUserShield, FaCode } from 'react-icons/fa';

const navLinks = [
  { href: '/careers', label: 'Role', icon: FaBriefcase },
  { href: '/projects/new', label: 'Start a Project', icon: FaPlusSquare },
  { href: '/services/website-design', label: 'Build a Website', icon: FaCode },
...
interface AppSubNavbarProps extends React.HTMLAttributes<HTMLDivElement> {
  initialIsExpanded: boolean;
  onCollapse: () => void;
  user: any;
  fullWidthContent?: boolean; // New prop to control content width
}

export default function AppSubNavbar({ initialIsExpanded, onCollapse, user, fullWidthContent = false, ...rest }: AppSubNavbarProps) {
  ...
  const linkContainerClasses = isExpanded 
    ? (isDesktop 
      ? 'flex flex-wrap items-center gap-2 p-2 justify-center' // Expanded desktop
      : 'grid grid-cols-2 gap-2 p-2') // Changed for expanded mobile grid 2x3
    : (isDesktop 
      ? 'flex flex-wrap items-center gap-2 p-2 justify-center' // Collapsed desktop
      : 'hidden'); // Collapsed mobile
  ...
  return (
    <div className={`bg-black text-gray-300 sticky top-[92px] z-40 transition-all duration-300 ease-in-out border-t border-gray-700`}>
      <div className={fullWidthContent ? "w-full px-2 sm:px-4" : "container mx-auto px-2 sm:px-4"}>
        {/* Welcome section */}
        {isExpanded && (
        ...
        <div className={`flex items-center py-2.5 sm:py-3 ${!isExpanded && !isDesktop ? 'hidden' : ''}`}> {/* Removed ml-auto, added mr-3 */}
          {/* Toggle button - MOVED to the far left for desktop */}
          {isDesktop && (
            <button 
              onClick={handleToggle} 
              className="p-1.5 sm:p-2 rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-600 flex-shrink-0 mr-3"
          ...
          <div className={`flex-grow ${linkContainerClasses}`}>
            {displayedLinks.map((link, index) => {
              ...
              if (link.label === 'Role') {
                return null; // Commenting out the Role button by returning null
              }
