...
import { sections as serviceSections } from './SubNavigation'; // Import sections from SubNavigation
    ...
    <header className="sticky top-0 z-40 w-full bg-white dark:bg-black border-b border-gray-200 dark:border-gray-800 text-black dark:text-white py-4 shadow-sm px-6">
      <div className="flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-6 flex-grow justify-start">
          {[...sectionLinks, ...serviceSections].map((link) => (
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {/* Always show Studio link */}
            <span key="studio-link" className="flex items-center gap-1">
              <Link href="/studio" className="text-sm font-medium text-gray-300 hover:text-white transition-colors">
                Studio
              </Link>
            </span>
            {isAuthenticated ? (
              // If authenticated, show Log out link
              <button onClick={handleLogout} className="text-sm font-medium text-gray-300 hover:text-white transition-colors">
                Log out
              </button>
            ) : (
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-gray-900 dark:bg-gray-900 border-t border-gray-800 dark:border-gray-800 shadow-lg py-4 px-4">
          ...
          <nav className="flex flex-col space-y-3 mb-4 items-start">
            {[...sectionLinks, ...serviceSections].map((link) => (
            ...
            {/* Always show Studio link in mobile menu */}
            <Link key="mobile-studio-link" href="/studio" className="block text-base font-medium text-gray-300 hover:text-white transition-colors" onClick={handleMobileLinkClick}>
              Studio
            </Link>
