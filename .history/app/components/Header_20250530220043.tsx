  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-background border-b border-border py-4 shadow-sm px-6">
      <div className="flex items-center justify-between">
        ...
        <Link key="desktop-studio-link" href="/studio" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors ml-6 md:ml-8">
          Studio
        </Link>
      ...
      {isMobileMenuOpen && (
        /* Keep specific gray shades for mobile menu background and border for contrast */
        /* If these styles clash, consider using background/border variables with opacity */
        <div className="md:hidden absolute top-full left-0 w-full bg-gray-900 border-t border-gray-800 shadow-lg py-4 px-4">
          ...
          <nav className="flex flex-col space-y-3 mb-4 items-start w-full">
            ...
            <Link key="mobile-studio-link" href="/studio" className="block text-base font-medium text-muted-foreground hover:text-foreground transition-colors w-full" onClick={handleMobileLinkClick}>
              Studio
            </Link>
            {sectionLinks.map((link) => (
              <Link key={`${link.name}-mobile`} href={link.href} className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
                {link.name}
              </Link>
            ))}
            ...
            {isAuthenticated ? (
              ...
              <button onClick={() => { handleLogout(); handleMobileLinkClick(); }} className="block text-base font-medium text-muted-foreground hover:text-foreground transition-colors mt-3">
                Log out
              </button>
            ) : (
              // If not authenticated, show original utility links in mobile menu
              utilityLinks.filter(link => link.name !== 'Studio').map((link) => (
                <Link key={`mobile-${link.name}`} href={link.href} className="block text-base font-medium text-muted-foreground hover:text-foreground transition-colors" onClick={handleMobileLinkClick}>
                  {link.name}
                </Link>
              ))
            )}
               {/* Admin Link (padlock icon) in mobile menu */}
               <Link href="/admin" aria-label="Admin Dashboard" className="block text-base font-medium text-muted-foreground hover:text-blue-400 transition-colors mt-2" onClick={handleMobileLinkClick}>
