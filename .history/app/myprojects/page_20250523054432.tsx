...
interface SortableProjectCardProps {
  ...
  editingProjectIdOnCard: string | null;
  currentEditingNameOnCard: string;
  isSavingProjectNameOnCard: boolean;
  onStartEditProjectNameOnCard: (projectId: string, currentName: string) => void;
  onCancelEditProjectNameOnCard: () => void;
  onSaveProjectNameOnCard: () => void;
  onCurrentEditingNameChangeOnCard: (newName: string) => void;
}
...
function SortableProjectCard({ 
  ...
  editingProjectIdOnCard,
  currentEditingNameOnCard,
  isSavingProjectNameOnCard,
  onStartEditProjectNameOnCard,
  onCancelEditProjectNameOnCard,
  onSaveProjectNameOnCard,
  onCurrentEditingNameChangeOnCard
        ...
        <div className="flex items-center gap-x-3 flex-wrap">
          ...
          <Link href={`/myprojects/${project.project_slug}`} legacyBehavior>
            <a className="text-xl font-semibold text-sky-400 hover:text-sky-300 hover:underline">
              {editingProjectIdOnCard === project.id ? (
                <input
                  type="text"
                  value={currentEditingNameOnCard}
                  onChange={(e) => onCurrentEditingNameChangeOnCard(e.target.value)}
                  onBlur={onSaveProjectNameOnCard}
                  className="text-xl font-semibold text-sky-400 hover:text-sky-300 hover:underline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 focus:ring-offset-slate-900"
                />
              ) : (
                <a className="text-xl font-semibold text-sky-400 hover:text-sky-300 hover:underline">
                  {project.name}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {projects.map(project => (
                  <SortableProjectCard 
                    ...
                    editingProjectIdOnCard={editingProjectIdOnCard}
                    currentEditingNameOnCard={currentEditingNameOnCard}
                    isSavingProjectNameOnCard={isSavingProjectNameOnCard}
                    onStartEditProjectNameOnCard={handleStartEditProjectNameOnCard}
                    onCancelEditProjectNameOnCard={handleCancelEditProjectNameOnCard}
                    onSaveProjectNameOnCard={handleSaveProjectNameOnCard}
                    onCurrentEditingNameChangeOnCard={setCurrentEditingNameOnCard}
                  />
                ))}
              </div>
            </SortableContext>
