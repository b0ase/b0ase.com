...
export default function MyProjectsPage() {
  ...
  const fetchProjects = useCallback(async (userId: string) => {
    ...
    if (!userId) {
      console.log("[MyProjectsPage] fetchProjects: No userId provided, setting projects to empty and isLoading to false.");
      setProjects([]);
      setIsLoading(false);
      return;
    }
    setIsLoading(true);
    setError(null);
    console.log("[MyProjectsPage] fetchProjects: Starting to fetch. isLoading set to true.");
    ...
    try {
      ...
      console.log("[MyProjectsPage] fetchProjects: Finally block reached. Setting isLoading to false.");
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    console.log("[MyProjectsPage] useEffect triggered. authUser:", authUser, "authLoading:", authLoading);
    if (authUser && !authLoading) {
      console.log("[MyProjectsPage] useEffect: authUser present and not authLoading. Calling fetchProjects and fetchUserPlatformRole.");
      fetchProjects(authUser.id);
      const fetchUserPlatformRole = async () => {
        console.log("[MyProjectsPage] fetchUserPlatformRole called.");
        if (!authUser || !authUser.id) { // Check authUser.id explicitly
          console.log("[MyProjectsPage] fetchUserPlatformRole: authUser or authUser.id is missing. authUser:", authUser);
          return;
        }
        try {
      ...
      console.log("[MyProjectsPage] useEffect: Conditions not met (authUser likely not yet available or auth still loading). authUser:", authUser, "authLoading:", authLoading);
      // Only clear projects and stop loading if auth is explicitly finished and there's no user.
      // If authLoading is true, we wait. If authLoading is false and authUser is null, then it's safe to say no user.
      if (!authLoading && !authUser) {
        console.log("[MyProjectsPage] useEffect: Auth loading finished AND no user. Clearing projects, setting isLoading to false.");
        setProjects([]);
        setIsLoading(false);
      }
      // If authLoading is still true, isLoading remains true, and we wait for another effect run when authUser or authLoading changes.
      // If authLoading is false but authUser became available in this cycle, the main 'if' block would have run.
    }
  ...
  const handleSaveNewProjectName = async () => {
    ...
    // Ensure authUser and authUser.id are available and id is a string
