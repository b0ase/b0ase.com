...
export default function TeamPage() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      // Here you would typically also call a function to persist the new order to your backend
      // For now, it's client-side only
      // Example: updateProjectOrder(arrayMove(managedProjects, oldIndex, newIndex));
      console.log('Project order changed (client-side): ', arrayMove(managedProjects, managedProjects.findIndex(item => item.id === active.id), managedProjects.findIndex(item => item.id === over.id)));
    }
  };
